...
void createMesh(vector<vector<float>>& input,float xres, float yres,float max,vector<int>& indicies, vector<Vertex>& vertexes)
{
  ...
  for(int i = 0; i < input.size()-1; i++)
  {
    ...
    {  
        ...
        vectors.push_back(Vertex{ {i*xres,UL,j*yres}, a, {i/input.size(),j/input[i].size()} } );
        vectors.push_back(Vertex{ {(i+1)*xres,LL,j*yres}, c, {(i+1)/input.size(),j/input[i].size()} } );
        vectors.push_back(Vertex{ {i*xres,UR,(j+1)*yres}, b, {i/input.size(),(j+1)/input[i].size()} } );
        vectors.push_back(Vertex{ {(i+1)*xres,LR,(j+1)*yres}, d, {(i+1)/input.size(),(j+1)/input[i].size()} } );
